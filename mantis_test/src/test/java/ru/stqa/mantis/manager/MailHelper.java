...
import jakarta.mail.*;
import ru.stqa.mantis.model.MailMessages;
...
public class MailHelper extends HelperBase {
    ...
    public List<MailMessages> receive(String username, String password, Duration duration) {
        ...
        while (System.currentTimeMillis() < start + duration.toMillis()) {
            try {
                ...
                var result = Arrays.stream(messages)
                        .map(m -> {
                            try {
                                return new MailMessages().withFrom(m.getFrom()[0].toString())
